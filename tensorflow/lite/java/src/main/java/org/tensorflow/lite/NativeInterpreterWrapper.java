...
class NativeInterpreterWrapper implements AutoCloseable {
  ...
  NativeInterpreterWrapper(String modelPath) {
    this(modelPath, /* options= */ null, null);
  }

  NativeInterpreterWrapper(ByteBuffer byteBuffer) {
    this(byteBuffer, /* options= */ null, null);
  }

  NativeInterpreterWrapper(String modelPath, InterpreterImpl.Options options, long[] resolverPtr) {
    TensorFlowLite.init();
    long errorHandle = createErrorReporter(ERROR_BUFFER_SIZE);
    long modelHandle = createModel(modelPath, errorHandle);
    init(errorHandle, modelHandle, options, resolverPtr);
  }

  NativeInterpreterWrapper(ByteBuffer buffer, InterpreterImpl.Options options, long[] resolverPtr) {
    TensorFlowLite.init();
    if (buffer == null
  ...
  private void init(long errorHandle, long modelHandle, InterpreterImpl.Options options, long[] resolverPtr) {
    if (options == null) {
