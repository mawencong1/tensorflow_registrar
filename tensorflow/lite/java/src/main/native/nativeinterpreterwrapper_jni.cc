...
Java_org_tensorflow_lite_NativeInterpreterWrapper_createInterpreter(
  ...
  std::unique_ptr<tflite::MutableOpResolver> resolver = tflite::CreateOpResolver();
  jsize registererLength = env->GetArrayLength(resolverPtr);
  jlong* registerers_by_func = env->GetLongArrayElements(resolverPtr, nullptr);
  for (jsize i = 0; i < registererLength; ++i) {
      //        uintptr_t resolver_ptr = reinterpret_cast<std::function<void(uintptr_t)>>(registerers_by_func[i]);
      //        void (*funcPtr)() = reinterpret_cast<void (*)()>(funcPtrAsLong);
      auto resolver_ptr = reinterpret_cast<void (*)(uintptr_t)>(registerers_by_func[i]);
      //        RegistererFunctionType registerer = reinterpret_cast<RegistererFunctionType>(registerers_by_func[i]);
      resolver_ptr(reinterpret_cast<uintptr_t>(resolver.get()));
  }
  ...
  env->ReleaseLongArrayElements(resolverPtr, registerers_by_func, 0);
